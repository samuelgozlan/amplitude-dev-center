...
| `delete_from_org`                  | Delete user from the entire org instead of a single project. This feature is available in orgs with the Portfolio feature enabled. Requests must be by `user_ids` and must exists in all projects. This option can be combined with `ignore_invalid_id` field to skip projects that are missing the given `amplitude_ids`. Values can be either `True` or `False`. Defaults to `False`.                                                                                                                        |
